...
import NormalCellType from "../../../../tables/data/selector/normal-cell";
import EditableCell from "../../../../tables/data/selector/editable-cell/editable-cell";

export default function EditableTableFile({ tableModel, list, loading }) {
    ...
    const saveData = async (newData, call) => {
        ...
        record.file_id = tableModel.foreignKey;
        const body = {
            // eslint-disable-next-line react/prop-types
    ...
    for (let i = 0; i < tableModel.headers.length; i++) {
        ...
        if (i === 0) {
            columns.push({
                ...
                dataIndex: tableModel.headers[i].indexKey,
                editable: false,
                width: "1%",
                render: (_, record) => (
                    <div style={{ width: 35 }}>
            ...
            columns.push({
                title: tableModel.headers[i].name,
                dataIndex: tableModel.headers[i].hide?"ff":tableModel.headers[i].indexKey,
                width: tableModel.headers[i].hide?0:"18%",
                editable: true,
                render:  (text,record) => {
